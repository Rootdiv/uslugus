...
function dataURLtoFile(base64, id) {
  ...
  writeFile(`./img/${id}.${ext}`, base64Image, { encoding: 'base64' }, err => {
  ...
  if (!/^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[\W]).{6,}/.test(item.password)) {
    errors.push({
      ...
      message:
        ...
        'Минимальная длина пароля 6 символов, обязательно содержит спецсимвол число и латинские буквы в нижнем и вернем регистре',
    });
  }
...
function createItems(data) {
  const id = Math.random().toString(10).substring(2, 8) + Date.now().toString(10).substring(9);
  ...
  try {
    const body = await (async () => {
      if (uri === '' || uri === '/') {
        ...
        if (req.method === 'PATCH') return updateItems(itemId, await drainJson(req));
